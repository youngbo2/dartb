...
```
Tableau가 데이터를 읽기 힘든 경우 (빈 셀, 머릿글, 바닥글, 메모 등의 이유로)
```
...
![image](https://github.com/user-attachments/assets/fd810ccd-4f47-4fb1-a90b-e476d8721516)
...
```
라이브는 태블로가 데이터 원본에 직접 연결되는 방식, 추출은 hyper라는 방식으로 데이터 파일을 따로 저장한다. 라이브는 원본 데이터의 수정사항이 바로 반영되지만 속도가 느리다.
```
...
```
관계로 데이터를 결합할 때 원본 데이터를 더 독립적으로 활용할 수 있다.
관계로 결합된 두 테이블은 단일 테이블로 병합된 것이 아니다.
관계는 원래 테이블의 세부 수준을 유지할 수 있게 한다.
태블로에서는 대부분의 경우 관계 결합을 권장한다.
그러나 제어 혹은 의도적인 필터링, 복제를 위해 조인을 하는 경우도 있다.
```
...
```
조인을 할 때에는 테이블을 캔버스로 끌고 온 후 열기
조인 아이콘을 클릭하면 조인 유형 선택 가능
...
<!-- 캡쳐 이미지를 첨부해주세요 -->
![image](https://github.com/user-attachments/assets/f0c98460-86c8-4d4a-97b0-ececdb425e2e)
* "주문" 시트와 "반품 정리" 시트를 "주문 id"를 이용해 inner join 한다.
* 결과값은 3027
...
```
혼합은 관계나 조인과 달리 데이터를 실제로 결합하지는 않는다. 각 데이터 원본을 독립적으로 집계한 후에 한 시트에서 데이터를 시각화한다.
혼합을 하려면, 시트에 들어가서 데이터-> 새 데이터 원본 -> 파일 선택하고 다시 시트로 돌아오면 2개의 테이블이 열린다 -> 주문 테이블의 지역을 열에, 반품 테이블의 주문 id를 마크창에 넣는다.
```
...
* 데이터의 테이블 구조가 동일해야 한다. 즉 필드수, 필드이름, 데이터 유형이 일치해야 한다.
...
* 라이브는 태블로가 데이터 원본에 직접 연결되는 방식, 추출은 hyper라는 방식으로 데이터 파일을 따로 저장한다. 라이브는 원본 데이터의 수정사항이 바로 반영되지만 속도가 느리다. 추출은 데이터 집계에 최적화되어있다. 대규모 데이터 세트, 필터, 계산 등이 포함된 경우 추출이 유리하다. 반면 실시간 데이터를 이용하는 경우 라이브를 사용한다.
...
```
데이터를 가지고 올 때 데이터 형식을 잘못 가져오는것을 방지하고 편하게 데이터 타입을 변경할 수 있다.
데이터 형식에 따라 다른 형태의 필터를 사용할 수 있다.
```
...
* 데이터들의 테이블 구조가 동일하므로 유니온 결합이 적절하다.
* `CARD_SUBWAY_MONTH_2024*`
