...
```
차원으로 분류되는 데이터는 정성적인 값을 가지고 있다. e.g.이름, 카테고리 등 불연속적
측정값은 정량적인 값을 가지고 있다. 연속적 데이터
```

*Quiz: 차원은 불연속형이고, 측정값은 연속형이다 (o, x)* -> 보통 그렇지만 시각화 방식에 따라서 달라질 수도 있다
...
```
자동으로 그룹바이를 해주는 것은 좋지만, 만약 데이터의 모양이 이상한 경우에는 어떻게 될지?
```
...
```
연속형 데이터라서 그라데이션으로 표현, 불연속형이라면 끊어져서 표현됨
```
...
```
측정값 이름을 열에 넣어주지 않으면 이중 축 그래프라서 할인율/매출이 겹쳐서 표시됨. 
하나의 월에 두 개의 막대 그래프를 표시하기 위해서
```
...
```
![image](https://github.com/user-attachments/assets/2439693b-7519-43ca-b85a-01b7a47087eb)

```
필터 -> 국가/지역 -> 대한민국
마크 -> 하위범주 -> 레이블 
마크 -> 매출 -> 색상, 크기
마크 -> 매출 -> 레이블 -> 다음을 사용하여 계산 -> 하위 범주
```
...
```
![image](https://github.com/user-attachments/assets/44bea934-664d-4307-b90b-0bb84d3cd71a)
