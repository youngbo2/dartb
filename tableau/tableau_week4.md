...
> 년-분기-월-일 / 국가-시도-도시 등 계층 구조를 나타낸다
>
> 계층 만들려면 2가지 이상의 필드 선택하고 우클릭-> 계층만들기

## 31. 집합

<!-- 집합의 정의 및 활용 방법에 대해 알게 된 점을 적어주세요. -->
> 집합은 사용자가 어떤 조건을 설정하고 그 조건을 기반으로 데이터를 구분하는 방법
>
> 필드 우클릭 -> 만들기 -> 집합
...
> 기존 데이터 이외에 데이터를 계산하여 새 필드를 만든다
>
> 필드 -> 우클릭 -> 만들기 -> 계산된 필드 -> 수식 작성
...
>
> 필드 -> 우클릭 -> 기본속성 -> 집계 / 또는 계산된필드 sum()함수를 통해 만들기
>
> 두 방식 차이점 있음: 직접 만든 필드는 집계 방법 변경 등 불가능

## 36. 테이블계산

<!-- 테이블 계산의 개념 및 사용 방법을 적어주세요. -->
> 테이블 계산은 뷰에 보이는 내용을 바탕으로 데이터가 계산.
>
> 계산된 필드 만들기 -> 테이블 계산 -> 적용 후 마크에서 우클릭 -> 다음을 이용하여 계산 설정 가능 or 테이블 계산 편집

## 37. 퀵테이블계산(1)

<!-- 퀵테이블 계산의 원리 및 예제에 대해 알게 된 점을 적어주세요. -->
> ? : 테이블 계산 기능을 클릭만으로 작동할 수 있게 하는 기능
>
> 행/마크 -> 우클릭 -> 퀵 테이블 계산 -> 누계, 차이 등 적용 가능, 적용된 필드 더블클릭시 함수 확인 가능

## 38. 퀵테이블계산(2)

<!-- 이동평균, YTD 총계, 전년 대비 성장률, YTD 성장률 등 본 강의에서 알게 된 점을 적어주세요. -->
> * 이동평균: 이전의 값부터 현재까지 값에 대한 평균 -> 퀵테이블계산
> * YTD 총계: 특정 시점을 기준으로 해당 연도부터 그 시점까지 총계, 기본적으로 누계와 같은 개념이지만 하위 필드가 있어야 사용가능
...
![image](https://github.com/user-attachments/assets/20fe6f04-7acf-49a1-838f-50927b3a7440)

* 매출상위 20개 시/도 집합 생성
* 수익률 계산된 필드 작성
* 수익률 조건이 마이너스인( < 0) 집합 생성
* 두 집합 교집합 결합 -> 마크창 색상
...
```
어떤 주문 ID들이 여러 개의 제품 레코드를 가지고 있어서 걸린 일 수를 전부 합계해버림
```

그리고 이를 해결하기 위해서는 어떻게 해야 할까요?

```
해당 필드 (주문까지 걸린 일수)를 측정값에서 차원값으로 변경
```
